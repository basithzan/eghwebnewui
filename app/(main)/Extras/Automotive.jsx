      ...
      <div className="px-[5%] pb-10">
        <div className="md:py-5 py-2 flex items-center md:justify-end justify-center">
          <div className="text-[#282828] md:text-end text-center text-sm">
            <a href="/">Home</a> /{" "}
            <a href="/group-of-companies">Group of Companies</a> / {title}
          </div>
        </div>

        <div className="section-spaced text-center">
          <div className="text-2xl lg:text-6xl font-semibold mb-10 md:mt-6 mt-6 uppercase">
            {title}
          </div>
          {descriptions.map((description, index) => (
            <p
              key={index}
              className="mb-2 md:mb-3 text-[#282828] text-[1rem] lg:text-[1.2rem]"
            >
              {description}
            </p>
          ))}
          <a
            href="/"
            className={`w-fit block mx-auto bg-white hover:bg-[#fb511e] text-black hover:text-white transition-all border border-1 border-black hover:border-[#fb511e] rounded-full px-10 py-3 button-1 font-light mt-16`}
          >
            Visit Website
